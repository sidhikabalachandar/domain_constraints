// this model lets sigma float but fixes alpha_risk = 1. 
// r = X * beta_Y + Z
...
transformed parameters{
    ...
    for(j in 1:N_zeroed_out_beta_delta_indices){
      if(zeroed_out_beta_delta_indices[j] != -1){
        masked_beta_delta[zeroed_out_beta_delta_indices[j]] *= 0;
      }
    }
}
model {
    ...
    vector[N] XbT = XbY + X * masked_beta_delta; // model is T ~ sigmoid(XbT + Z) = sigmoid(X*beta_Y + X*beta_delta + Z) = sigmoid(r + X * beta_delta)
